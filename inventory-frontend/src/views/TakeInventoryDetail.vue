          ...
          <li v-for="inventory in userInventory" :key="inventory.id">
            <b-row class="inventory-row">
              ...
              <b-col cols="auto">
                <button
                  class="check-button"
                  v-on:click="clickConfirm(inventory.itemNo)"
                  :disabled="inventory.checked"
                >
                  <span v-if="inventory.checked">{{$t('confirmed')}}</span>
...
export default {
  data() {
    return {
      ...
      invertoryFormDes:
        "周一是南韓1919年三一獨立運動102周年，總統文在寅在首爾的紀念活動發表講話，談及韓日及兩韓關係。他表示，今年夏季的日本東京奧運會可為南韓、北韓、美國、日本提供對話契機，又指韓方準備好隨時近年因二戰慰安婦問題而交惡的日本對話。",
      selectedItemNo: null,
  ...
  methods: {
    ...
    deleteItem() {
      ...
      this.userInventory = lodash.remove(this.userInventory, function(
        invertory
      ) {
